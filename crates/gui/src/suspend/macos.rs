//! macOS suspend detection via `NSWorkspaceWillSleepNotification`.
//!
//! Cannot be exercised by a unit test — there is no way to make macOS post
//! a real sleep notification from a test process. Verify manually: run
//! `cargo run -p gui` on a real Mac (or a macOS VM with sleep support),
//! then run `pmset sleepnow` from a terminal and confirm Nox locks before
//! the display goes dark (add a temporary `eprintln!("suspend: will sleep")`
//! at the top of the closure below if the lock isn't visually obvious with
//! the screen already off).
...
/// `()` on `tx` once per suspend event, for the remaining process lifetime.
///
...
pub(crate) fn watch_will_sleep(tx: UnboundedSender<()>) {
